...
def import_edge(tx, from_name, to_name, edge_type, label):
    tx.run(f"MERGE (a:{label} {{name: {from_name}}})", from_name=from_name)
    tx.run(f"MERGE (b:{label} {{name: {to_name}}})", to_name=to_name)
    tx.run(
        f"MATCH (a:{label} {{name: {from_name}}}), (b:{label} {{name: {to_name}}}) "
